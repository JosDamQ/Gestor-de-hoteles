...
exports.saveRoom = async (req, res) => {
  try {
    ...
    if (role != hotelExist.admin)
      return res
        .status(400)
        .send({ message: "Your not the admin of this hotel" });
...
exports.getsRoomsByHotel = async (req, res) => {
  try {
    ...
    let rooms = await Room.find({ hotel: hotelId }).populate("hotel");
    if (!rooms)
      return res.status(400).send({ message: "Not rooms found in this hotel" });
...
exports.updateRoom = async (req, res) => {
  try {
    ...
    if (role != room.hotel.admin /*.toString()*/)
      return res.status(401).send({ message: "Your not admin of this hotel" });
    if (room.available == false)
      return res.status(400).send({ message: "This room is ocupated" });
    let existNumber = await Room.findOne({
      number: data.number,
      hotel: room.hotel,
    });
    if (existNumber && existNumber._id.toString() !== roomId)
      return res.status(400).send({ message: "Room number already exist" });
    await Room.updateOne({ _id: roomId }, params1, { new: true });
